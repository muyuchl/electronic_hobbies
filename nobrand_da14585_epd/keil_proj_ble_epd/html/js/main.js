...
function reverseByteBits(byte) {
  ...
  // 遍历8个比特位
  for (let i = 0; i < 8; i++) {
      // 将当前位左移到目标位置(7-i)，并与结果进行或操作
      result |= ((byte >> i) & 1) << (7 - i);
  }
  return result;
}

function hori_flip_image_data(srcImgArr, width, height) {
  arr = new Array(srcImgArr.length).fill(0)
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
        ...
        srcX = width - 1 - x;
        srcBitPos = 7 - (srcX % 8)
        if (srcImgArr[y * bytesPerLine + Math.floor(srcX/8)] & (1 << srcBitPos)) {
            // set pixel
            arr[y * bytesPerLine + Math.floor(x/8)] |= (1 << bitPos)
...
async function sendimg() {
  ...
  if (imgArray.length === ramSize * 2) {
    ...
    const newImgArray = hori_flip_image_data(imgArray, 128, 250)
  await epdWrite( 0x24 , newImgArray);
