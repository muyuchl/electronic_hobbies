...
void user_svc1_led_wr_ind_handler(ke_msg_id_t const msgid,
                                     ...
                                     ke_task_id_t const src_id)
{
    user_epd_handle_ble_request(&param->value[0], param->length);
    
    
            
    /*
    if (param->length > DEF_SVC1_LED_STATE_CHAR_LEN) {
        // invalid, 
        g_epd_command_len = 0;
        return;
    }
    ...
    g_epd_command_len = param->length;
    memcpy(g_epd_command_buf, &param->value[0], param->length);
...
//    sample = (sample <= 0xffff) ? (sample + 1) : 0;
    ...
    req->length = g_epd_command_len;
    req->notification = true;
    memcpy(req->value, g_epd_command_buf, g_epd_command_len);
...
void user_epd_send_response(uint8_t *response, uint16_t length)
{
    // not used in web epd project
    #if 0
     struct custs1_val_ntf_ind_req *req = KE_MSG_ALLOC_DYN(CUSTS1_VAL_NTF_REQ,
                                                          prf_get_task_from_id(TASK_ID_CUSTS1),
                                                          TASK_APP,
    ...
    req->length = length;
    req->notification = true;
    memcpy(req->value, response, length);
